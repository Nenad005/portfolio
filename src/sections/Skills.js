            ...
            {skills.map((skill, index) => {
                return <div key={index} id="skill" className="flex flex-col w-fit justify-center items-center">
                    <div className="w-fit h-fit group border-solid border-borderColor border-[1px] p-5 rounded-3xl">
                        <img className="object-contain group-hover:animate-pulse duration-300 group-hover:scale-110 w-[60px] h-[60px] md:w-[70px] md:h-[70px]" src={`${backendUri}${skill.attributes.Image.data.attributes.url}`}
